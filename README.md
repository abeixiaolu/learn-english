...
├── README.md           # 本说明
├── PLAN.md             # 学习计划与语法单元大纲
├── .cursor/skills/     # daily-study / review-answer / mistake-recap / weekly-review
...
3. **全部做完后**，用 `/review-answer` 批改（错题会自动追加解析）。
4. 用 `/mistake-recap` 在当天文件末尾追加错题回顾。
5. 新一天：用 `/daily-study` 按 `PLAN.md` 生成下一天的 `day-XXX.md`。
6. 每 7 天：用 `/weekly-review` 生成复习报告。
...
| `/review-answer` | 批改当天 Part 1–4 作答 | 练习做完后 |
| `/mistake-recap` | 在 day 文件末尾追加错题回顾 | 批改后回顾 |
...
（自己做题，答案写在文件里）
/review-answer        → 批改，看错误模式
/mistake-recap        → 错题回顾（追加在 day 文件末尾）
